% ChemmineR
% Yiqun Cao, Tyler Backman, Kevin Horan, Thomas Girke 
...
```
<font color='red'>Porting this vignette to knitr is causing some formatting issues. This will be
 resolved soon.</font>
...
![*ChemmineR* environment with its add-on packages and
...
version of this package was published in 
`r citet(c("10.1093/bioinformatics/btn307"))` . Since then many additional
utilities and add-on packages have been added to the environment and
many more are under development for future releases (Figure 2; 
`r citep(c("10.1093/nar/gkr320","10.1093/bioinformatics/btt475"))`).
...
-   Integration of a subset of OpenBabel functionalities via new
    *ChemmineOB* add-on package `r citep(c("10.1093/bioinformatics/btn307"))`
...
![Visualization webpage created by calling
*sdf.visualize*.](visualizescreenshot.png )
...
functionalities implemented in the OpenBabel C++ library 
`r citep(c("http://journal.chemistrycentral.com/content/2/1/5", "10.1093/bioinformatics/btn307" ))` . These
...
-   *convertFormatFile*: converts files between two
    formats
...
300. We then fetch the matching compounds and show their molecular
weight. 
```{r eval=TRUE, tidy=FALSE}
...
ring perception algorithm from `r citet(c("10.1021/ci960322f"))` . In addition, the function can return
all smallest possible rings as well as aromaticity information.
...
browser using the online ChemMine Tools service. The service allows to
display other information next to the structures using the extra
argument of the *sdf.visualize* function. The following
examples demonstrate, how one can plot and annotate structures by
passing on extra data as vector of character strings, matrices or
lists.  
...
[](http://www.bioconductor.org/packages/devel/bioc/html/fmcsR.html)
...
[](http://www.bioconductor.org/packages/devel/bioc/vignettes/fmcsR/inst/doc/fmcsR.pdf)
and the associated publication is available in 
`r citet(c("10.1093/bioinformatics/btt475"))`. The following gives a
...
one or many compounds 
`r citep(c("10.1021/ci00046a002","10.1021/ci025531g"))` . It returns a searchable atom pair database
...
*as(matrix, "FPset")* and *as(character,
"FPset")* construct an *FPset* database from
...
Construct *FPset* class form *character
vector*: 
```{r eval=TRUE, tidy=FALSE}
...
structures of the matching compounds along with their scores.  
...
cheminformatics `r citep("http://ieeexplore.ieee.org/xpl/articleDetails.jsp?arnumber=1672233")`. It requires a nearest neighbor table, which consists
...
Foundation:
[](http://www.nsf.gov/awardsearch/showAward.do?AwardNumber=0957099),
